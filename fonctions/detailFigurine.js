...
if (cardData.charaUrl.length >= 1 && typeof cardData.charaUrl != "string") {
  for (let i = 0; i < cardData.charaUrl.length; i++) {
    const img = new Image();
    img.onload = () => {
      imagesHtmla += `<img class="chara" src="${cardData.charaUrl[i]}" width="50%">`;
...
if (cardData.inspiUrl.length >= 1 && typeof cardData.inspiUrl != "string") {
  for (let i = 0; i < cardData.inspiUrl.length; i++) {
    const img = new Image();
    img.onload = () => {
      imagesHtmlb += `<img class="inspi" src="${cardData.inspiUrl[i]}" width="50%">`;
...
if (
  ...
  for (let i = 0; i < cardData.visualisationUrl.length; i++) {
    const img = new Image();
    img.onload = () => {
      imagesHtmlc += `<img class="inspi" src="${cardData.visualisationUrl[i]}" width="50%">`;
